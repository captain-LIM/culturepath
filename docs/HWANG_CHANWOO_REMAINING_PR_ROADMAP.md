...
`7개 → 12개`로 개선됐다. 강릉·전주 책방은 검색 로직만으로 채울 수 없는 TourAPI 원천
데이터 공백이 확인됐다. R16.4는 검증된 6개 문화의 공식 분류 코드 우선 조회를 자동
테스트로 추가했으며 별도 live smoke는 수행하지 않았다.
...
## 3. R17 — AI 구조 전환과 여행 챗봇(구현·CI 완료)
...
11. Qdrant·embedding의 활성 요청 경로 의존 제거. 수동 이력 스크립트는 `decay` 계약과
    함께 보존
...
## 6. 완료 계약 — R16.2·R16.3·R16.4와 하단 5탭
...
공식 코드와 일대일 대응되는 6개 문화는 `lclsSystm1~3` 후보를 우선하고 기존 키워드를
함께 사용한다. 엄격 검증 결과가 5개 미만이거나 공식 조회가 첫 페이지 또는 후속
페이지에서 실패하면 분류 조건 없는 지역 목록으로 보충한다. 나머지 4개 문화는 기존
지역 목록과 복수 검색어 방식을 유지한다. 상세 계약은
[TourAPI 장소 계약](./TOUR_PLACE_CONTRACT.md)을 따른다.

### Flutter 하단 내비게이션

현행 앱의 root 목적지는 `홈 / 탐색 / 만들기 / AI / 내정보` 5개다. 일반 AI 진입은
`/ai`, 코스 문맥의 `AI로 다듬기`는 같은 화면에 `courseId`를 전달한다. R14·R15의 4탭
문서는 당시 디자인 이력이며 현행 내비게이션 계약으로 사용하지 않는다.
