...
| 2 | R12 | **다음 작업·계획 필요** | 관광지 이미지·상세·연관 장소 수직 연결 | R11 공개 목록 계약 |
...
- R11 문화별 관광지 관련도·정렬 품질은 구현됐으며, 다음 구현 단위는 **R12 — 관광지 이미지·상세·연관 장소 수직 연결**이다.
...
- 사용자가 명시적으로 `구현 시작`하기 전에는 R12 코드를 수정하지 않는다.
