...
| AI 같지 않은 디자인 | 정보구조·휴먼 검수 기준 문서 완료, 실제 Manus 프로토타입·Figma 원본 미승인 | R14·R15 |
...
| 3 | R14 | **설계 문서 완료·Manus 생성/검수 대기** | Manus 프로토타입·Figma P0 디자인 확정 | R11·R12의 실제 데이터 상태 |
| 4 | R15 | 대기 | Flutter 디자인 적용·모바일 디자인 QA | R14 디자인 승인 |
...
R11·R12의 실제 데이터 상태를 기준으로 화면 구조와 시각 언어를 확정한다. Manus에서 독립형 Android 프로토타입을 충분히 생성·수정하고, 승인 결과만 Figma 원본으로 정리한 뒤 사람이 설계한 제품처럼 검수한다.
...
- Figma 무료 AI 크레딧은 승인된 Manus 결과의 토큰·컴포넌트·Variant·Prototype 정리에 우선 사용한다.
...
- 팀원 요청의 홈·탐색 구조가 상태별로 명세된다.
- Figma 컴포넌트와 토큰을 Flutter로 모호함 없이 옮길 수 있다.
- Manus 프로토타입과 최종 Figma 원본이 생성되고 검수표의 차단 항목이 0개다.
- 황찬우가 최종 시안을 승인한 뒤에만 R15 구현 범위를 확정한다.
...
- R14의 최신 코드 감사, P0 정보구조·토큰·화면·상태 명세, Manus·Figma 프롬프트와 2회 검수표는 `agent/r14-manus-figma-p0-design` 브랜치에 작성됐다.
- 다음 행동은 황찬우가 디자인 명세와 검수표를 Manus에 첨부하고 [Manus 실행 프롬프트](./R14_MANUS_PROTOTYPE_PROMPTS.md)의 0번부터 순서대로 실행하는 것이다.
- Manus에서 승인된 화면만 [Figma 최종 정리 프롬프트](./R14_FIGMA_MAKE_PROMPTS.md)로 옮긴다. 최종 Figma 승인 전에는 R14 완료나 R15 시작으로 표시하지 않는다.
