...
| R14·R15 | PR #17·#18 | Manus/Figma P0 계약과 Flutter 디자인 적용·자동 모바일 QA |
| 팀원 지도 기능 | `67a1943`, `8195d41`, `64fcfc2` | 코스 Day별 Google Map, 좌표 저장·응답, 최신 코스 재조회, 지도 pan 수정 |

2026-08-24 재검증 기준:

- R16 변경 포함 Backend 자동 테스트 `257/257` 통과
...
- 최신 번역 캐시·코스 이미지 migration 적용 완료
- live fixture 13개 `contentId`를 TourAPI에서 MySQL로 적재하고 공식 감사 `13/13`, issue `0`, pending evidence `0` 통과
...
- 기존 Mock fixture·Mock title·Qdrant schema는 변경하지 않았다. 일반 박물관 규칙은 유지하고 실제 국문 상세로 근대사 장소임을 검증한 `1684836`, `2607311`만 `CONTENT_ID_OVERRIDES`에 추가했다.
...
| 4 | R15 | **머지 완료(PR #18), 실기기 수동 QA는 R18** | Flutter 디자인 적용·자동 모바일 디자인 QA | R14 조건부 승인과 인계 계약 |
| 5 | R16 | **구현·자동검증·MySQL 감사·독립리뷰 완료** | 실제 TourAPI 기준 RAG 평가 계약 재정의 | R11 분류 정책·실제 MySQL 표본 |
| 6 | R16.1 | **구현·live smoke·독립리뷰 완료, 커밋 전** | 문화 장소 복수 검색어·안전한 분류 보완·최대 50건 페이지 계약 | 14개 실제 표본 감사 |
...
황찬우 실행 순서는 **R16.1 TourAPI 탐색 보완 → 후속 Flutter 무한 스크롤 디자인 → OpenRouter → 배포**다. 다국어와 지도 고도화는 임수민 담당으로 별도 진행하며, 황찬우는 머지 후 디자인·RAG 계약 호환성만 확인한다. 한 번에 하나의 코드 PR만 구현한다.
...
- `rag-evaluation-live-v1.json`은 최소 15개 case를 요구하며 실제 MySQL 감사 후 12개 relevance와 3개 `coverage_gap`으로 확정했다.
...
- `npm run rag:seed-live-fixture -- --live`는 명시적 live 실행에서 누락 ID만 TourAPI 국문 상세로 적재하고, fixture 장소의 저장 culture를 현재 검증 규칙과 동기화한다.
- `npm run rag:audit-live-fixture`로 fixture ID와 `places_cache`의 culture·locale별 title을 외부 API 없이 대조하고 MySQL pool을 종료한다.
- 일반 박물관 규칙은 유지하고 실제 근대사 장소로 검증된 `1684836`, `2607311`만 `근대 문화유산` override로 등록한다.
- Qdrant schema, OpenRouter 모델과 Flutter 계약은 변경하지 않는다.

### 현재 제한

- 로컬 MySQL에 번역 캐시·코스 이미지 migration을 순서대로 적용했고, TourAPI 실제 국문 상세으로 13개 fixture ID를 모두 적재했다.
- 2026-08-24 공식 감사는 13/13 발견, issue 0, pending evidence 0으로 통과했고 case 증거는 `mysql_verified`다.
- quality gate는 아직 `baseline`이므로 `evidenceVerified=true`여도 `contractReady=false`, `ready=false`다.
...
## 9.1 R16.1 — 문화 장소 재현율·페이지 계약

### 근거와 범위

- 2026-08-25 대표 14개 지역×문화 제한 감사에서 기존 계약은 7개 조합만 비어 있지 않았다.
- `통영국제음악당`은 정확한 검색에서 정상 음악 분류됐지만 대표어 `공연장`으로 발견되지 않았다.
- 전라감영양조장·안동소주·군산 책방은 TourAPI에 있지만 공식 대분류 제한으로 제목 근거가 무시됐다.
- 문화별 검색어를 최대 3개로 확장한다. 첫 후보 페이지가 부족할 때만 보조 검색어를 순차 활성화하고, 다음 페이지도 같은 소스 순서를 누적 재조회해 중복·누락을 막는다.
- `AC + 서점/책방`, `EX·VE + 전통주/양조장/소주`만 강한 제목 근거와 함께 허용하며 `EV` 행사는 제외한다.
- 같은 14개 표본 재검증은 비어 있지 않은 조합이 7개에서 12개로 늘었고, 강릉·전주 책방 2개만 0건으로 남았다.
- 전주·포항은 두 구를 각각 조회해 합치고 다른 시·군 장소가 섞이지 않도록 지역 경계를 보정한다.
- `/regions/:code/spots`는 기본 20개·최대 50개와 다음 페이지 헤더를 지원한다. 통합 후보 재조회는 TourAPI 호출 증폭을 막기 위해 최대 5페이지로 제한한다. Flutter 무한 스크롤과 공개 코스 사용 횟수 랭킹은 별도 후속 PR이다.
- Backend 응답 배열·TourAPI ID·MySQL schema는 바꾸지 않는다.

### 완료 조건

- 14개 제한 표본을 같은 조건으로 재검증해 개선·잔여 공백을 기록한다.
- 일부 후보 조회 실패는 검증된 부분 결과로 복구하고, 전부 실패할 때만 공통 외부 API 오류를 반환한다.
- Backend 전체 테스트와 독립 `gpt-5.6-sol high` 리뷰를 통과한다.
...
황찬우: R11·R12 완료 → R14 Manus 프로토타입·Figma P0 → R15 Flutter 디자인 QA
                                      ↓
                 R16 Mock/live RAG 평가 → R16.1 TourAPI 탐색 → R17 OpenRouter live → R18 배포
...
- 현재 작업 브랜치는 `feat/r16-1-culture-place-recall`이며 TourAPI 문화 장소 재현율과 최대 50건 페이지 계약 구현·검증을 마쳤고 커밋 전이다.
- 기존 Mock 35개 fixture는 그대로이며 live baseline 15개 case는 `contentId` 정답과 명시적 coverage gap을 사용한다.
- 로컬 MySQL migration, 13개 live fixture 적재와 공식 `places_cache` 감사는 완료됐다. R16.1 완료 뒤 Flutter 무한 스크롤 시점 또는 R17 OpenRouter 준비 여부를 판단한다.
- 병합 전 Backend 자동 테스트는 `270/270` 통과했고, 구현에 참여하지 않은 `gpt-5.6-sol high` 최종 리뷰는 blocker/required 지적 없이 `APPROVE` 판정을 받았다.
