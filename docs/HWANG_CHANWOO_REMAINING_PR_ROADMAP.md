...
| 2 | R12 | **구현 완료·검증 중** | 관광지 이미지·상세·연관 장소 수직 연결 | R11 공개 목록 계약 |
| 3 | R13 | **다음 작업·R12 머지 후 계획** | 다국어 관광지 데이터 계약과 연결 | R12 장소 모델·상세 화면 |
...
- R11 문화별 관광지 관련도와 R12 이미지·상세·연관 장소 수직 연결은 구현됐다. R12 검증·머지 후 다음 구현 단위는 **R13 — 다국어 관광지 데이터 계약과 연결**이다.
...
- R12는 `agent/r12-place-images-detail` 브랜치에서 작업하며 Flutter 로컬 도구가 없어 GitHub CI의 analyze/test 확인이 필수다.
