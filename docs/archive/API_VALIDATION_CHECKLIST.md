...
| 영문 관광정보 서비스 | `https://apis.data.go.kr/B551011/EngService2` | 공식 서비스·Base URL 확인, 미신청·미호출 | 활용 신청 후 목록·검색·상세·이미지 검증 |
| 일문 관광정보 서비스 | `https://apis.data.go.kr/B551011/JpnService2` | 공식 서비스·Base URL 확인, 미신청·미호출 | 활용 신청 후 목록·검색·상세·이미지 검증 |
| 중문 간체 관광정보 서비스 | `https://apis.data.go.kr/B551011/ChsService2` | 공식 서비스·Base URL 확인, 미신청·미호출 | 활용 신청 후 목록·검색·상세·이미지 검증 |
| 중문 번체 관광정보 서비스 | `https://apis.data.go.kr/B551011/ChtService2` | 공식 서비스·Base URL 확인, 미신청·미호출 | 활용 신청 후 목록·검색·상세·이미지 검증 |
...
- [x] 광역·기초 응답을 공통 내부 방문자 모델로 정규화
- [x] 소수 방문자 수, 결측 수치, 실제 달력 날짜, 코드·방문자 유형 유효성 검증
- [x] 원본 `numOfRows`·`totalCount` 기준 전체 페이지 수집과 최대 5페이지 상한
- [x] 페이지 메타데이터 일관성, 실제 수집 건수와 방문자 복합키 중복 검증
- [x] 현재 큐레이션 지역만 내부 slug와 명시적으로 매핑
- [x] 강원·전북 행정구역 개편 전·후 코드 alias 처리
- [x] 기준일과 7일 전 동일 요일의 외지인·외국인 합계 변화 지표 계산
- [x] 전용 MySQL 캐시의 7일 fresh·30일 stale, 캐시 조회 포함 single-flight와 DB fail-open
- [x] 불완전한 응답·코드 누락·외부 장애 시 문화 목록 전체 큐레이션 fallback
...
- 자동 재시도 `0`, 실제 호출 정확히 `4회`
- `20260723`: 기초·광역 모두 HTTP/API 정상 처리, `totalCount=0`
- `20260716`: 기초·광역 모두 HTTP/API 정상 처리, `totalCount=0`
- 키와 전체 URL은 출력하지 않음
- 최근 날짜에 지역명이 없어 UTF-8 한글은 이번 smoke에서 재확인하지 못함
- 호출 상한에 따라 다른 날짜를 추가 탐색하지 않고 실제 데이터가 검증된 `20210513`을 고정 기준일로 유지
...
- [x] 네 언어 서비스의 공식 제공 여부와 현재 Base URL 확인
- [ ] 공공데이터포털에서 네 서비스 활용 신청 및 승인 상태 확인
- [ ] 각 서비스의 목록·키워드 검색·공통 상세·이미지 API 최소 1회 실호출
- [ ] HTTP `200`뿐 아니라 `resultCode=0000`, JSON, UTF-8, 해당 언어 제목·주소·개요 확인
- [ ] 한국어 장소와 번역 장소가 같은 `contentId`를 공유하는지 실제 표본으로 확인
- [ ] `contentId`가 다르면 지역 코드·좌표·정규화 제목 중 무엇으로 동일 장소를 연결할지 계약 확정
- [ ] 번역 데이터가 없거나 이미지가 없는 장소의 한국어 fallback 정책 검증
- [ ] 언어를 포함한 MySQL 캐시 키로 한국어·영어·일본어·중국어 응답이 섞이지 않는지 검증
- [ ] Flutter locale을 백엔드 요청에 전달하고 공개 API 응답 언어가 실제로 바뀌는지 통합 테스트
- [ ] 이용 조건·출처 표시·이미지 저작권 구분값을 언어별 서비스에서도 보존하는지 확인

### 7.2 구현 전 고정할 계약

- UI 문구 locale과 관광지 콘텐츠 locale은 별개 상태로 다루되 같은 사용자 언어 선택을 입력으로 사용한다.
- 요청 언어 후보는 `ko`, `en`, `ja`, `zh-CN`, `zh-TW`다. 현재 Flutter의 단일 `zh`를 간체로 매핑할지, 간체·번체 선택으로 분리할지는 R13 착수 전에 결정한다.
- 번역 응답이 없을 때 빈 화면을 만들지 않고 한국어 원문으로 fallback하며, 화면에서 원문임을 구분할 수 있게 한다.
- locale은 장소 목록·상세·검색 캐시 키에 반드시 포함한다. 포함하지 않으면 먼저 저장된 언어가 다른 사용자에게 반환될 수 있다.
- 장소 동일성은 실호출 표본에서 ID 계약을 확인하기 전까지 제목 문자열만으로 확정하지 않는다.

## 8. Swagger 검증 후 백엔드에서 다시 확인할 항목
...
7. [x] DataLab의 `areaCode`·`signguCode`를 현재 큐레이션 지역 모델과 매핑한다.
8. [x] DataLab 전용 캐시, 타임아웃, 호출량 제한, 키 마스킹을 적용한다.
9. 필요할 때 나머지 `contentTypeId`의 소개·반복정보 표본을 추가 검증한다.
10. [ ] 다국어 관광정보 네 서비스의 활용 신청·승인 상태를 확인한다.
11. [ ] 네 서비스의 목록·검색·상세·이미지 최소 표본을 키 노출 없이 검증한다.
12. [ ] 언어 간 장소 동일성, 번역 누락 fallback, locale별 캐시 격리 계약을 확정한다.
13. [ ] Flutter 언어 선택부터 TourAPI 응답 언어까지 통합 검증한다.
...
- [한국관광공사 영문 관광정보 서비스](https://www.data.go.kr/data/15101753/openapi.do)
- [한국관광공사 일문 관광정보 서비스](https://www.data.go.kr/data/15101760/openapi.do)
- [한국관광공사 중문 간체 관광정보 서비스](https://www.data.go.kr/data/15101764/openapi.do)
- [한국관광공사 중문 번체 관광정보 서비스](https://www.data.go.kr/data/15101769/openapi.do)
- [TourAPI 다국어 서비스 변경 공지](https://www.data.go.kr/bbs/ntc/selectNotice.do?originId=NOTICE_0000000004082)
