...
- `q` 없이 `culture`가 있으면 지역 일반 목록과 문화별 대표 키워드 검색을 각각 최대 1회 수행하고 `contentId`로 합친다.
...
- CulturePath 지역 slug와 선택 `culture`로 최대 20개 장소를 반환한다.
- `culture`가 있으면 지역 일반 목록과 대표 키워드 검색 결과를 합쳐 `/places/search`와 같은 판정·중복 제거·정렬을 적용한다.
- 선택 culture를 응답 `category`에 덮어쓰지 않고 재분류된 장소의 실제 대표 category를 유지한다.
- 일치 장소가 없으면 가상 seed로 개수를 채우지 않고 `200 []`를 반환한다.
- 문화 필터 요청에서 TourAPI가 실패하면 가상 `SPOT_MAP`으로 숨기지 않고 공통 `502`·`503`·`504` 오류 계약을 따른다.
