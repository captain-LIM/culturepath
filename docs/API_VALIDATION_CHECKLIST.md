...
- 해당 코드를 `KorService2`의 `contentid`와 직접 연결할 수 있는지
- 직접 연결되지 않을 경우 이름·지역 기반 보조 매핑에 필요한 필드
- 데이터가 없는 연월·지역 조합의 응답 형태
...
세 공공데이터 서비스의 공통 클라이언트와 TourAPI 구 지역 파라미터 호환 호출은 검증됐다. 목록·검색 서비스 구현은 현행 법정동 계약과 fixture 테스트를 기준으로 완료했지만, `lDongRegnCd`·`lDongSignguCd`를 사용하는 실제 호출은 아직 검증하지 않았다. 다음 승인된 smoke test에서 법정동 코드 조회와 현행 목록·검색 요청을 함께 확인한다.

1. [x] 공통 HTTP 클라이언트와 환경설정 모듈을 구현한다.
2. [x] 성공·빈 결과·최상위 오류 객체를 하나의 내부 응답 또는 오류 형식으로 정규화한다.
3. [ ] `ldongCode2`와 현행 법정동 파라미터의 실제 호출을 검증한다.
4. `searchKeyword2 → 상세 API` 서비스 흐름을 구현한다.
5. 연관 관광지의 해시 ID와 TourAPI 숫자형 `contentid` 매핑 전략을 정한다.
6. DataLab의 `areaCode`·`signguCode`를 내부 지역 모델과 매핑한다.
7. 캐시, 타임아웃, 호출량 제한, 키 마스킹을 적용한다.
8. 필요할 때 나머지 `contentTypeId`의 소개·반복정보 표본을 추가 검증한다.
